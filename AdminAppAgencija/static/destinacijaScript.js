...
function dodaj() {
  ...
  let grad = document.getElementById("novi_grad").value;
  let drzava = document.getElementById("nova_drzava").value;
...
function azuriraj() {
  let id = document.getElementById("novi_id").value;
  let ime = document.getElementById("novi_grad").value;
  let zvezdice = document.getElementById("nova_drzava").value;
  ...
  fetch("http://localhost:7000/admin/destinacija/" + id, {
    method: "PUT",
    headers: {
      "Content-Type": "application/json",
      // 'Authorization': 'Bearer ' + token
...
function add(novi) {
  ...
  td1.classList.add("destinacija-id");
  td2.classList.add("destinacija-Grad");
  td3.classList.add("destinacija-Drzava");

  let text1 = document.createTextNode(novi.id);
  let text2 = document.createTextNode(novi.grad);
  let text3 = document.createTextNode(novi.drzava);
